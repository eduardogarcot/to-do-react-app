...
const DELETE_BTN_CN = 'w-[2rem] h-[2rem] p-1 border border-solid border-slate-800 bg-white rounded hover:bg-slate-200 hover:cursor-pointer';
const CONTAINER_PROJECT_ITEM = 'flex p-4 border my-4 bg-slate-200 border-slate-400 first:mt-0 last:mb-0 items-center';


const ProjectItem = ({id, title, description, isSelected, handleEdit, handleDelete, handleSelectProject}) => {
    return ( <div className={CONTAINER_PROJECT_ITEM}>
        <Checkbox checked={isSelected} onChange={()=>handleSelectProject(id)}/>
